...
#r1 hoppings
        matrix[0,4] += -np.conj(self.t[2]) * np.exp(-1j * np.dot(k,r1)) #A1 to B1
        matrix[4,0] += np.conj(matrix[0,4]) #B1 to A1

        matrix[0,6] += -self.t[1] * np.exp(1j * np.dot(k,r1)) # A1 to B3
        matrix[6,0] += np.conj(matrix[0,6]) # B3 to A1

        matrix[1,5] += -self.t[1] * np.exp(-1j * np.dot(k,r1)) # A2 to B2
        matrix[5,1] += np.conj(matrix[1,5]) # B2 to A2

        matrix[1,7] += -np.conj(self.t[2]) * np.exp(1j * np.dot(k,r1)) # A2 to B4
        matrix[7,1] += np.conj(matrix[1,7]) # B4 to A2

        matrix[2,4] += -self.t[1] * np.exp(1j * np.dot(k,r1)) # A3 to B1
        matrix[4,2] += np.conj(matrix[2,4]) # B1 to A3
        
        matrix[2,6] += -np.conj(self.t[2]) * np.exp(-1j * np.dot(k,r1)) # A3 to B3
        matrix[6,2] += np.conj(matrix[2,6]) # B3 to A3

        matrix[3,5] += -np.conj(self.t[2]) * np.exp(1j * np.dot(k,r1)) # A4 to B2
        matrix[5,3] += np.conj(matrix[3,5]) # B2 to A4
        
        matrix[3,7] += -self.t[1] * np.exp(-1j * np.dot(k,r1)) # A4 to B4
...
#r3 hoppings
        matrix[4,8] += -np.conj(self.t[0]) * np.exp(1j * np.dot(k,r3)) #B1 to C1
        matrix[8,4] += np.conj(matrix[4,8]) #C1 to B1
        
        matrix[4,11] += -self.t[0] * np.exp(-1j * np.dot(k,r3)) #B1 to C4
        matrix[11,4] += np.conj(matrix[4,11]) #C4 to B1

        matrix[5,9] += -self.t[0] * np.exp(1j * np.dot(k,r3)) #B2 to C2
        matrix[9,5] += np.conj(matrix[5,9]) #C2 to B2  
        
        matrix[5,10] += -np.conj(self.t[0]) * np.exp(-1j * np.dot(k,r3)) #B2 to C3
        matrix[10,5] += np.conj(matrix[5,10]) #C3 to B2

        matrix[6,9] += -np.conj(self.t[0]) * np.exp(-1j * np.dot(k,r3)) #B3 to C2
        matrix[9,6] += np.conj(matrix[6,9]) #C2 to B3   

        matrix[6,10] += -self.t[0] * np.exp(1j * np.dot(k,r3)) #B3 to C3
        matrix[10,6] += np.conj(matrix[6,10]) #C3 to B3

        matrix[7,8] += -self.t[0] * np.exp(-1j * np.dot(k,r3)) #B4 to C1
        matrix[8,7] += np.conj(matrix[7,8]) #C1 to B4
        
        matrix[7,11] += -np.conj(self.t[0]) * np.exp(1j * np.dot(k,r3)) #B4 to C4
